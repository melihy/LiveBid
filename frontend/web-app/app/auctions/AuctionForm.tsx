...
export default function AuctionForm({ auction }: Props) {
  ...
  const {
    control, handleSubmit, setFocus, reset, formState: { isSubmitting, isValid, isDirty, errors },
    ...
    <form className="flex flex-col mt-3" onSubmit={handleSubmit(onSubmit)}>
      ...
      {pathname === '/auctions/create' &&
        <>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Input label="Reserve Price (enter 0 is no reserve)" name="reservePrice" control={control} type='number' rules={{ required: "Reserve Price is required" }} />
