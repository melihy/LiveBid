...
namespace AuctionService.IntegrationTests
{
    ...
    public class AuctionControllerTests : IAsyncLifetime
    {
        ...
        {
            //arrange?
            var auction = new CreateAuctionDto(); //required fields are empty
        ...
        public async Task UpdateAuction_WithValidUpdateDtoAndUser_ShouldReturn200()
        {
            // arrange
            _httpClient.SetFakeJwtBearerToken(AuthHelper.GetBearerForUser("bob"));
            var auction = await _httpClient.GetFromJsonAsync<AuctionDto>($"api/auctions/{_gT_ID}");
            auction.Make = "--updated-make--";

            // act
            var response = await _httpClient.PutAsJsonAsync($"api/auctions/{auction.Id}", auction);

            //assert
            response.EnsureSuccessStatusCode();
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            var updatedAuction = await _httpClient.GetFromJsonAsync<AuctionDto>($"api/auctions/{_gT_ID}");
            Assert.Equal("--updated-make--", updatedAuction.Make);
        }
        ...
        {
            // arrange
            _httpClient.SetFakeJwtBearerToken(AuthHelper.GetBearerForUser("--invalid-user--"));
            var auction = await _httpClient.GetFromJsonAsync<AuctionDto>($"api/auctions/{_gT_ID}");
            auction.Make = "--updated-make--";
            auction.Seller = "--updated-seller--";
